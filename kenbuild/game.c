...
int loadgame(void)
{
	...
	dfread(&numplayers,4,1,fil);
	dfread(&myconnectindex,4,1,fil);
	dfread(&connecthead,4,1,fil);
	dfread(connectpoint2,4,MAXPLAYERS,fil);

		//Make sure palookups get set, sprites will get overwritten later
	for(i=connecthead;i>=0;i=connectpoint2[i]) initplayersprite((short)i);

	dfread(posx,4,MAXPLAYERS,fil);
	dfread(posy,4,MAXPLAYERS,fil);
	dfread(posz,4,MAXPLAYERS,fil);
	dfread(horiz,4,MAXPLAYERS,fil);
	dfread(zoom,4,MAXPLAYERS,fil);
	dfread(hvel,4,MAXPLAYERS,fil);
	dfread(ang,2,MAXPLAYERS,fil);
	dfread(cursectnum,2,MAXPLAYERS,fil);
	dfread(ocursectnum,2,MAXPLAYERS,fil);
	dfread(playersprite,2,MAXPLAYERS,fil);
	dfread(deaths,2,MAXPLAYERS,fil);
	dfread(lastchaingun,4,MAXPLAYERS,fil);
	dfread(health,4,MAXPLAYERS,fil);
	dfread(numgrabbers,2,MAXPLAYERS,fil);
	dfread(nummissiles,2,MAXPLAYERS,fil);
	dfread(numbombs,2,MAXPLAYERS,fil);
	dfread(flytime,4,MAXPLAYERS,fil);
	dfread(oflags,2,MAXPLAYERS,fil);
	dfread(dimensionmode,1,MAXPLAYERS,fil);
	dfread(revolvedoorstat,1,MAXPLAYERS,fil);
	dfread(revolvedoorang,2,MAXPLAYERS,fil);
	dfread(revolvedoorrotang,2,MAXPLAYERS,fil);
	dfread(revolvedoorx,4,MAXPLAYERS,fil);
	dfread(revolvedoory,4,MAXPLAYERS,fil);

	dfread(&numsectors,2,1,fil);
	dfread(sector,sizeof(sectortype),numsectors,fil);
	dfread(&numwalls,2,1,fil);
	dfread(wall,sizeof(walltype),numwalls,fil);
		//Store all sprites (even holes) to preserve indeces
	dfread(sprite,sizeof(spritetype),MAXSPRITES,fil);
	dfread(headspritesect,2,MAXSECTORS+1,fil);
	dfread(prevspritesect,2,MAXSPRITES,fil);
	dfread(nextspritesect,2,MAXSPRITES,fil);
	dfread(headspritestat,2,MAXSTATUS+1,fil);
	dfread(prevspritestat,2,MAXSPRITES,fil);
	dfread(nextspritestat,2,MAXSPRITES,fil);

	dfread(&fvel,4,1,fil);
	dfread(&svel,4,1,fil);
	dfread(&avel,4,1,fil);

	dfread(&locselectedgun,4,1,fil);
	dfread(&loc.fvel,1,1,fil);
	dfread(&oloc.fvel,1,1,fil);
	dfread(&loc.svel,1,1,fil);
	dfread(&oloc.svel,1,1,fil);
	dfread(&loc.avel,1,1,fil);
	dfread(&oloc.avel,1,1,fil);
	dfread(&loc.bits,2,1,fil);
	dfread(&oloc.bits,2,1,fil);

	dfread(&locselectedgun2,4,1,fil);
	dfread(&loc2.fvel,sizeof(input),1,fil);

	dfread(ssync,sizeof(input),MAXPLAYERS,fil);
	dfread(osync,sizeof(input),MAXPLAYERS,fil);

	dfread(boardfilename,1,80,fil);
	dfread(&screenpeek,2,1,fil);
	dfread(&oldmousebstatus,2,1,fil);
	dfread(&brightness,2,1,fil);
	dfread(&neartagsector,2,1,fil);
	dfread(&neartagwall,2,1,fil);
	dfread(&neartagsprite,2,1,fil);
	dfread(&lockclock,4,1,fil);
	dfread(&neartagdist,4,1,fil);
	dfread(&neartaghitdist,4,1,fil);

	dfread(turnspritelist,2,16,fil);
	dfread(&turnspritecnt,2,1,fil);
	dfread(warpsectorlist,2,16,fil);
	dfread(&warpsectorcnt,2,1,fil);
	dfread(xpanningsectorlist,2,16,fil);
	dfread(&xpanningsectorcnt,2,1,fil);
	dfread(ypanningwalllist,2,64,fil);
	dfread(&ypanningwallcnt,2,1,fil);
	dfread(floorpanninglist,2,64,fil);
	dfread(&floorpanningcnt,2,1,fil);
	dfread(dragsectorlist,2,16,fil);
	dfread(dragxdir,2,16,fil);
	dfread(dragydir,2,16,fil);
	dfread(&dragsectorcnt,2,1,fil);
	dfread(dragx1,4,16,fil);
	dfread(dragy1,4,16,fil);
	dfread(dragx2,4,16,fil);
	dfread(dragy2,4,16,fil);
	dfread(dragfloorz,4,16,fil);
	dfread(&swingcnt,2,1,fil);
	dfread(swingwall,2,32*5,fil);
	dfread(swingsector,2,32,fil);
	dfread(swingangopen,2,32,fil);
	dfread(swingangclosed,2,32,fil);
	dfread(swingangopendir,2,32,fil);
	dfread(swingang,2,32,fil);
	dfread(swinganginc,2,32,fil);
	dfread(swingx,4,32*8,fil);
	dfread(swingy,4,32*8,fil);
	dfread(revolvesector,2,4,fil);
	dfread(revolveang,2,4,fil);
	dfread(&revolvecnt,2,1,fil);
	dfread(revolvex,4,4*16,fil);
	dfread(revolvey,4,4*16,fil);
	dfread(revolvepivotx,4,4,fil);
	dfread(revolvepivoty,4,4,fil);
	dfread(subwaytracksector,2,4*128,fil);
	dfread(subwaynumsectors,2,4,fil);
	dfread(&subwaytrackcnt,2,1,fil);
	dfread(subwaystop,4,4*8,fil);
	dfread(subwaystopcnt,4,4,fil);
	dfread(subwaytrackx1,4,4,fil);
	dfread(subwaytracky1,4,4,fil);
	dfread(subwaytrackx2,4,4,fil);
	dfread(subwaytracky2,4,4,fil);
	dfread(subwayx,4,4,fil);
	dfread(subwaygoalstop,4,4,fil);
	dfread(subwayvel,4,4,fil);
	dfread(subwaypausetime,4,4,fil);
	dfread(waterfountainwall,2,MAXPLAYERS,fil);
	dfread(waterfountaincnt,2,MAXPLAYERS,fil);
	dfread(slimesoundcnt,2,MAXPLAYERS,fil);

		//Warning: only works if all pointers are in sector structures!
	dfread(tmpanimateptr,4,MAXANIMATES,fil);
	for(i=MAXANIMATES-1;i>=0;i--)
		animateptr[i] = (int *)(tmpanimateptr[i]+(intptr_t)sector);

	fread(animategoal,4,MAXANIMATES,fil);
	fread(animatevel,4,MAXANIMATES,fil);
	fread(animateacc,4,MAXANIMATES,fil);
	...
	fread(pskyoff,2,MAXPSKYTILES,fil);
	fread(&pskybits,2,1,fil);

	fread(&mirrorcnt,2,1,fil);
	fread(mirrorwall,2,mirrorcnt,fil);
	fread(mirrorsector,2,mirrorcnt,fil);
